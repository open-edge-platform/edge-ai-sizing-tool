...
import { RefreshCw, MemoryStick, ServerOff } from 'lucide-react'
...
import { GpuMemoryChartProps } from '@/types/gpu-types'
...
export function GpuMemoryChart({
  ...
  const [chartData, setChartData] = useState<
    { time: string; gpuMemoryUsage: number }[]
  >([])
    ...
    try {
      if (device) {
        setChartData((prevData) => {
          const newData = [
            ...prevData,
            { time: new Date().toLocaleTimeString(), gpuMemoryUsage: value },
          ]
          ...
          <CardContent>
            <div className="flex h-40 flex-col items-center justify-center py-3">
              ...
              <p className="text-muted-foreground text-center text-xs">
                Fetching graphics processor memory metrics...
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="w-full">
          <CardContent>
            <div className="flex h-40 flex-col items-center justify-center">
              ...
              />
              <p className="mb-1 text-center text-sm font-medium">
                Failed to load GPU memory data
              </p>
          ...
          <ChartContainer className="h-30 w-full" config={chartConfig}>
            ...
            >
              ...
              <Area
                ...
                type="monotone"
                stroke={chartConfig.gpuMemoryUsage.color}
                fill={chartConfig.gpuMemoryUsage.color}
                fillOpacity={0.5}
                strokeWidth={2}
              />
            </AreaChart>
