...
async function getGPUInfo(): Promise<{ name: string; device: string }[]> {
    ...
    for (const available_device of available_devices) {
        if (available_device.startsWith("GPU")) {
            const name = String(core.getProperty(available_device, "FULL_DEVICE_NAME"))
            gpus.push({
                id: available_device,
                name: name,
...
export async function GET() {
    try {
        ...
        const memory =  (typeof memInfo.total === "number") ? bytesToGigabytes(memInfo.total) : NOT_AVAILABLE
        const freeMemory =  (typeof memInfo.free === "number") ? bytesToGigabytes(memInfo.free) : NOT_AVAILABLE
        const freeMemoryPercentage = (typeof memory === "number" && typeof freeMemory === "number") ? calculatePercentage(freeMemory, memory): NOT_AVAILABLE
        const usedMemory = (typeof memory === "number" && typeof freeMemory === "number") ? Math.round((memory - freeMemory) * 10) / 10 : NOT_AVAILABLE
        const usedMemoryPercentage = (typeof usedMemory === "number" && typeof memory === "number") ? calculatePercentage(usedMemory, memory) : NOT_AVAILABLE

        const freeDisk = (typeof disk.available === "number") ? bytesToGigabytes(disk.available) : NOT_AVAILABLE
        const totalDisk =  (typeof disk.size === "number") ? bytesToGigabytes(disk.size) : NOT_AVAILABLE 
        const usedDisk = (typeof disk.used === "number") ? bytesToGigabytes(disk.used) : NOT_AVAILABLE
        const freeDiskPercentage = typeof freeDisk === "number" && typeof totalDisk === "number" ? calculatePercentage(freeDisk, totalDisk) : NOT_AVAILABLE
        const usedDiskPercentage = typeof usedDisk === "number" && typeof totalDisk === "number" ? calculatePercentage(usedDisk, totalDisk) : NOT_AVAILABLE
