...
const ASSETS_PATH = path.resolve(process.env.ASSETS_PATH ?? path.join(process.cwd(), "../assets/media"))
...
export async function POST(request: Request) {
    try {
        ...
        if (!securePath) {
            return new Response(JSON.stringify({ error: "Path validation failed" }), { status: 400 })
        }
        ...
        try {
            ...
            return new Response(JSON.stringify({ error: "Failed to create directory" }), { status: 500 })
        }

        try {
            await fs.promises.writeFile(securePath, Buffer.from(await file.arrayBuffer()))
        } catch (err) {
            console.error("Failed to write file:", err)
            return new Response(JSON.stringify({ error: "Failed to write file" }), { status: 500 })
        }

        return new Response(JSON.stringify({ file: uniqueFileName }), { status: 200 })
    } catch (error) {
