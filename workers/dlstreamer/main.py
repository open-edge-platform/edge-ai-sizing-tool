...
os.environ["GST_PLUGIN_PATH"] = "/opt/intel/dlstreamer/build/intel64/Release/lib:/opt/intel/dlstreamer/gstreamer/lib/gstreamer-1.0:/opt/intel/dlstreamer/gstreamer/lib/"
os.environ["LD_LIBRARY_PATH"] = "/opt/intel/dlstreamer/gstreamer/lib:/opt/intel/dlstreamer/build/intel64/Release/lib:/opt/intel/dlstreamer/lib/gstreamer-1.0:/usr/lib:/opt/intel/dlstreamer/build/intel64/Release/lib:/opt/opencv:/opt/openh264:/opt/rdkafka:/opt/ffmpeg:/usr/local/lib/gstreamer-1.0:/usr/local/lib"
os.environ["LIBVA_DRIVERS_PATH"] = "/usr/lib/x86_64-linux-gnu/dri"
os.environ["GST_VA_ALL_DRIVERS"] = "1"
os.environ["PATH"] = f"/opt/intel/dlstreamer/gstreamer/bin:/opt/intel/dlstreamer/build/intel64/Release/bin:{os.environ['PATH']}"
...
async def lifespan(app: FastAPI):
    ...
    app.state.pipeline_metrics = {"total_fps": None, "number_streams": None, "average_fps_per_stream": None, "timestamp": None}
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='FastAPI server for Intel® DLStreamer object detection model')
    parser.add_argument('--input', type=str, default=f"{VIDEO_DIR}/people-detection.mp4", help='Input source e.g. /dev/video0, videofile.mp4, etc')
    parser.add_argument('--inference_mode', type=str, default="gvadetect", help='Inference mode: gvadetect or gvaclassify (default: gvadetect)')
    parser.add_argument('--model', type=str, default="yolo11n", help='Model name (default: yolo11n)')
    parser.add_argument('--model_parent_dir', type=str, default=MODEL_DIR, help=f'Path to the model directory (default: {MODEL_DIR})')
    parser.add_argument('--model_precision', type=str, default='FP16', help='Model precision (default: FP16)')
    parser.add_argument('--device', type=str, default='CPU', help='Device to run inference on (default: CPU)')
    parser.add_argument('--decode_device', type=str, default='CPU', help='Device to run decode on (default: CPU)')
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size for inference (default: 1)')
    parser.add_argument('--tcp_port', type=int, default=5000, help='Port to spawn the DLStreamer pipeline (default: 5000)')
    parser.add_argument('--port', type=int, default=5997, help='Port to run the FastAPI server on (default: 5997)')
    parser.add_argument("--id", type=int, help="Workload ID to update the workload status")
    return parser.parse_args()

args = parse_arguments()

def build_pipeline(tcp_port, input, inference_mode, model_name, model_precision, device, decode_device, batch_size=1):
    """
    Build the DLStreamer pipeline for MJPEG streaming.
    """
    model_parent_dir = Path(args.model_parent_dir) / f"{model_name}-{model_precision}" / "1" / f"{model_name}.xml"
    ...
    if "CPU" in decode_device:
        if input.startswith("/dev/video"):
            ...
            decode_element = ["rtph264depay", "!", "avdec_h264", "!", "videoconvert", "!", "video/x-raw"]
    elif "GPU" in decode_device:
        decode_element = ["rtph264depay", "!", "avdec_h264", "!", "vapostproc", "!", "video/x-raw(memory:VAMemory)"]
    else:
    ...
    pipeline = (
        ...
        + ["!"]
        + inference_command
        + ["!", "queue", "!", "gvafpscounter", "!", "gvawatermark", "!", "videoconvert", "!", "jpegenc", "!", "multipartmux", "boundary=frame", "!", "tcpserversink", f"host=127.0.0.1", f"port={tcp_port}"]
    )
...
def filter_result(output):
    ...
    """
    fps_pattern = re.compile(r'FpsCounter\(average.*\): total=(\d+\.\d+) fps, number-streams=(\d+), per-stream=(\d+\.\d+) fps')
    ...
    for attempt in range(retries):
        try:
            ...
            if cap.isOpened():
                ...
                logging.warning(f"RTSP stream is not running at: {rtsp_url}. Retrying... ({attempt + 1}/{retries})")
        except Exception as e:
...
def main():
    ...
    if os.path.realpath(args.input) != os.path.abspath(args.input): # Check if the model path is a symlink
        logging.info(f"Error: Input file {args.input} is a symlink or contains a symlink in its path. Refusing to open for security reasons.")
    ...
    if not os.path.exists(args.input):
        ...
        ffmpeg_command = [
            "ffmpeg", "-re", "-stream_loop", "-1", "-i", args.input,
    ...
    model_status = export_yolo_model(model_name=args.model, model_parent_dir=args.model_parent_dir)
...
def get_mjpeg_stream():
    ...
    """
    try:
        return StreamingResponse(mjpeg_stream(port=args.tcp_port), media_type="multipart/x-mixed-replace; boundary=frame")
    except Exception as e:
        return JSONResponse({"status": False, "message": "An error occurred while retrieving mjpeg stream"})
...
def get_pipeline_metrics():
    ...
    try:
        ...
        return JSONResponse({"status": False, "message": "An error occurred while retrieving metrics"})
