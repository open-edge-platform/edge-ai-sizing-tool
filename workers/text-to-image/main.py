...
CUSTOM_MODEL_DIR = Path("../custom_models/text-to-image")
...
def setup_model(args: argparse.Namespace, env: Dict[str, str]):
        ...
        try:
            if args.repo_source == "modelscope":
                ...
                if not is_openvino_model:
                    additional_args = {"task": "text-to-image-with-past"}
                ...
                if is_openvino_model:
                    ...
                    optimum_cli(args, model_path, env)
    ...
    try:
        PIPE = openvino_genai.Text2ImagePipeline(str(model_path), args.device)
